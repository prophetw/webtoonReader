...
async function fetchComicsMetaInfo() {
...
function updateComicScore(score) {
  if (curComicName) {
    ...
    fetch(`${baseUrl}/api/updateScores/${curComicName}`, {
      ...
      warning('评分更新成功', 2000, 'top');
      const meta = comicMetaInfo[curComicName];
      if (meta) {
        meta.score = score;
      }
    });
  ...
  fetch(`${baseUrl}/api/comics`)
    ...
    .then(async (comics) => {
      ...
      comics.forEach((comic) => {
        ...
        summaryEleMap.set(comic, summary);
        if (comic) {
...
async function fetchEpisodes(comicName) {
  showLoading();
...
function warning(message, duration, position = 'mid') {
...
function loadNextEpisode(needConfirm = false) {
  stopSmoothScroll();
  if (needConfirm) {
    confirm('自动播放下一集', '3秒之后自动播放下一集', 3000).then(
      (isConfirmed) => {
        if (isConfirmed) {
...
function initSettings() {
  ...
  saveTagEle.addEventListener(action, () => {
    ...
    const summaryEle = summaryEleMap.get(curComicName);
		console.log(' summaryEle --- ', summaryEle);
    if (summaryEle) {
      const meta = comicMetaInfo[curComicName];
      const { score } = meta;
      summaryEle.innerHTML = `${curComicName} <span class="tags">${tags.join(',')}</span> <span class="score">评分：${score}</span>`;
    }
  });
